...
def historic_rates(request, base='USD', target='EUR', months=24):
    ...
    sql = ('select '
           ...
           'currency_name, '
           'currency_code '
           'from rates_currency where currency_code = %s ')
    ...
    sql = ('select '
           ...
           'from rates_rate target_rates '
           'join rates_rate base_rates on base_rates.rate_date = target_rates.rate_date '
           "where target_rates.currency_id = (select currency_id from rates_currency where currency_code = %s) "
           " and base_rates.currency_id = (select currency_id from rates_currency where currency_code = %s) "
...
def current_rates(request, base='USD'):
    ...
    sql = ('select '
           ...
           'from rates_rate target_rates '
           'join rates_rate base_rates on base_rates.rate_date = target_rates.rate_date '
           'join rates_currency target_currency on target_currency.currency_id = target_rates.currency_id '
           "where base_rates.currency_id = (select currency_id from rates_currency where currency_code = %s) "
           ' and target_rates.rate_date = (select max(rate_date) from rates_rate) '
